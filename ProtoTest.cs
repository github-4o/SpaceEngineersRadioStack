...
public class ProtoTest {
    ...
    /*!
     *  \brief emulates antenna failure. set to lesser values to speed up the test
     */
    readonly TimeSpan loopPeriod = new TimeSpan (0,0,0,0,100);
    /*!
     *  \brief constraints test run time
     */
    ...
    public void run () {
        Console.WriteLine (
            ...
            + "(on the downside, it's synchronous)");
        test1(false);
        test2(false);
    }
    /*!
     ...
     *  \param canSend flag allowing a ProtoStack to send messages
    ...
    void protoTestLoop (
        ...
        if (timeBudgetExceeded){ // watchdog check
            throw new Exception ("time budget exceeded");
        }

        // self-check: number of runs against number of measurements
        if (stepDureations.Count != cntSteps*2) {
            throw new Exception ("test self-check failed: stepDureations.Count");
        }

        // self-check: number of runs with canSend==true against number of measurements
        if (parsersDureations.Count != cntMsgs*2) {
            throw new Exception ("test self-check failed: parsersDureations.Count");
        }
        ...
        Console.WriteLine ("________________________________________"
            ...
            + "average step duration: " + (stepDureations.Average() / Stopwatch.Frequency)
            ...
            + "\naverage parser duration: " + (parsersDureations.Average() / Stopwatch.Frequency)
